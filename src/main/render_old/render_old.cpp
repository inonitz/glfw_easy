...
int render_particles_old()
{
    ...
    constexpr u32 particleCount = 4096;
    ...
    auto init_lib = []() -> u8 {
        u8 ctxid;
        init();
        ifcrash( (ctxid = Context::allocate() ) == 0);
        ...
        Context::init(
            ...
            WindowDescriptor{ {{ 720u, 480u }}, nullptr }
        );
        return ctxid;
    };
    ...
    contextID = init_lib();
    render.createFrom({
        { "src/main/render_old/bshader.vert", GL_VERTEX_SHADER   },
        { "src/main/render_old/bshader.frag", GL_FRAGMENT_SHADER }
    });
        ...
        ](std::vector<ParticleData>& particles) -> void 
    {
        ...
        auto scale_factor = k_unitRectangleLength;
