...
void sim_grid::update()
{
    // __rdirprintf("sim_grid::update()::begin() ... ");
    ...
    // __rdirprintf("sim_grid::update()::end()\n");
    return;
}


void sim_grid::destroy() {
...
void sim_grid::print_sortedIndices()
{
    u32 begin, end;
    __rdirprintf("sim_grid::print_sortedIndices()::begin() ");
    for(size_t active = 1; active <= m_activeIndicesSize; ++active) {
        begin = m_indices[m_activeIndices[active]    ];
        end   = m_indices[m_activeIndices[active] + 1];
...
void sim_grid::countOccurences(std::vector<i32>& flattenIndex)
{
...
void sim_grid::computePartialSums()
{
    u32 partialSum = 0;
...
void sim_grid::populateDenseArray_getActiveIndices(std::vector<i32>& flattenIndex)
{
...
#ifdef __rdirprintf
void sim_grid::test_countOccurences(std::vector<math::vec2i>& indices)
{
    const math::vec2i trunc1d = { __scast(i32, m_width), 1 };
    math::vec2f temp;
    for(size_t i = 0; i < m_data->size(); ++i) {
...
void sim_grid::test_computePartialSums()
{
    u32 partialSum = 0;
    for(auto& index : m_indices) {
        partialSum += index;
...
void sim_grid::test_populateDenseArray_getActiveIndices(std::vector<math::vec2i>& indices)
{
    const math::vec2i trunc1d = { __scast(i32, m_width), 1 };
    u16 finalIndex;
    for(size_t i = 0; i < m_data->size(); ++i) {
